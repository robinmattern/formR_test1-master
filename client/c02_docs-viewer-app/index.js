...
async function showDocInfo() {
    ...
    try {
        ...
        console.log(`  Error loading document info: ${error.message}`);
    }
}
    ...
    if (confirm(`Are you sure you want to delete "${currentDoc.title}"?`)) {
        try {
            ...
            if (response.ok) {
                ...
                document.getElementById('downloadBtn').style.display = 'none';
                document.getElementById('deleteBtn').style.display = 'none';
                document.getElementById('viewer').innerHTML = 'Select a document and click "View" to display its contents here.';
                currentDoc = null;
